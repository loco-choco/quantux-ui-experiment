...
func _notification(what):
	if what == NOTIFICATION_RESIZED:
		_position_popup(item.panel.get_global_rect())

func _process(_delta: float) -> void:
	if item.is_picked:
		request_deletion.emit()

func _position_popup(rect: Rect2) -> void:
	var pos: Vector2 = rect.position + Vector2(rect.size.x, 0)
	if not get_viewport_rect().encloses(Rect2(pos, size)):
		pos = rect.position - Vector2(size.x, 0)
	global_position = pos
